...
Vercel과 같은 클라우드 IP는 YouTube에서 자막 요청이 차단될 수 있습니다. 배포 환경에서는 `youtube-transcript-api`가 지원하는 회전형 주거용 프록시를 설정하세요.
...
- `503`: `OPENAI_API_KEY` 미설정 또는 배포 서버 IP의 YouTube 접근 차단
